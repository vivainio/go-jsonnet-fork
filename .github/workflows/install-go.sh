...
curl -fL -o "${TDIR}/go1.24.8.linux-amd64.tar.gz" 'https://go.dev/dl/go1.24.8.linux-amd64.tar.gz'
...
printf '%s %s/go1.24.8.linux-amd64.tar.gz\n' "${GO_DIST_SHA256}" "${TDIR}" | sha256sum -c

>&2 echo "Unpacking to /usr/local/go"
rm -rf /usr/local/go && tar -C /usr/local -xzf "${TDIR}/go1.24.8.linux-amd64.tar.gz"
